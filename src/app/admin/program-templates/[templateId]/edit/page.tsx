...
import { initialMockTemplates, updateMockProgramTemplate } from '@/app/admin/program-templates/page.tsx'; // Import shared data and update function
...
export default function EditProgramTemplatePage() {
  ...
  const [currentTemplateName, setCurrentTemplateName] = useState<string>('Loading...');
  ...
  useEffect(() => {
    if (templateId) {
      ...
      setTimeout(() => {
        const fetchedTemplate = initialMockTemplates.find(t => t.id === templateId); 
        if (fetchedTemplate) {
          setCurrentTemplateName(fetchedTemplate.name);
          form.reset({
            name: fetchedTemplate.name,
            description: fetchedTemplate.description || '',
            category: fetchedTemplate.category || '',
            status: fetchedTemplate.status || 'template',
            exercises: fetchedTemplate.exercises.map(ex => ({
                exerciseId: ex.exerciseId,
                sets: ex.sets !== undefined ? String(ex.sets) : "",
                reps: ex.reps !== undefined ? String(ex.reps) : "",
          ...
          toast({ variant: 'destructive', title: 'Error', description: 'Program template not found.' });
  ...
  }, [exerciseSearchTerm, fields, initialMockExercises]); // Added initialMockExercises
  ...
  }, [initialMockExercises]); // Added initialMockExercises

  const onSubmit = async (data: EditTemplateFormValues) => {
    setIsLoading(true);
    
    const templateToUpdate = initialMockTemplates.find(t => t.id === templateId);
    if (templateToUpdate) {
      ...
      updateMockProgramTemplate(updatedData); // Update central mock store
      console.log('Updated program template data (simulated save):', updatedData);
    }
  ...
  if (initialLoading) {
    return <div className="flex justify-center items-center h-64">Loading template data...</div>;
  }
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <Card className="shadow-md">
            <CardHeader>
              <CardTitle className="text-2xl font-headline text-primary flex items-center">
                <Layers className="mr-3 h-7 w-7" /> Edit Program Template: {currentTemplateName}
              </CardTitle>
              <CardDescription>Modify the details for this program template.</CardDescription>
            </CardHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="Briefly describe this template..." {...field} value={field.value || ''} rows={3} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="e.g., Post-Surgery, Shoulder, Spine" {...field} value={field.value || ''} /></FormControl>
          ...
          <Card className="shadow-md">
            <CardHeader>
              ...
              <CardDescription>Modify exercises and their parameters for this template.</CardDescription>
            </CardHeader>
              ...
              {fields.length === 0 && (
                ...
                render={() => ( 
                    ...
                    {fields.map((fieldItem, index) => {
                       const exerciseDetail = getExerciseDetails(fieldItem.exerciseId);
                       return (
                        <Card key={fieldItem.id} className="mb-4 p-4 relative group bg-background">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              <h4 className="font-semibold">{exerciseDetail?.name || `Exercise ID: ${fieldItem.exerciseId}`}</h4>
                          ...
                          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
                             <FormField
                                ...
                                render={({ field }) => (
                                    ...
                                    <FormControl><Input placeholder="e.g., 3" {...field} value={field.value || ''} /></FormControl>
                                    </FormItem>
                                )}
                            />
                            <FormField
                                ...
                                render={({ field }) => (
                                    ...
                                    <FormControl><Input placeholder="e.g., 10" {...field} value={field.value || ''} /></FormControl>
                                    </FormItem>
                                )}
                            />
                             <FormField
                                ...
                                render={({ field }) => (
                                    ...
                                    <FormControl><Input placeholder="e.g., 30s" {...field} value={field.value || ''} /></FormControl>
                                    </FormItem>
                                )}
                            />
                             <FormField
                                ...
                                render={({ field }) => (
                                    ...
                                    <FormControl><Input placeholder="e.g., 60s" {...field} value={field.value || ''} /></FormControl>
                                    </FormItem>
                                )}
                            />
                          </div>
                          <FormField
                                ...
                                render={({ field }) => (
                                    ...
                                    <FormControl><Textarea placeholder="Specific instructions for this exercise..." {...field} value={field.value || ''} rows={2}/></FormControl>
                                    </FormItem>
                                )}
                            />
                        </Card>
